...
def _copy_data(executor, src, zero=True, hole=True, progress=None):
    ...
    the qcow2 are never deallocated when zeoring.

    When copying to qcow2 image with a backing file, holes must not zeroed,
    ...
    When copying to new empty image wihout a backing file, we can optimize the
