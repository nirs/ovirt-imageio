...
# Limit maximum zero and copy size to speard the workload better to multiple
...
def copy(src_factory, dst_factory, dirty=False, max_workers=MAX_WORKERS,
         buffer_size=BUFFER_SIZE, zero=True, progress=None, name="copy"):

    buffer_size = min(buffer_size, MAX_BUFFER_SIZE)

    handler_factory = partial(
        Handler, src_factory, dst_factory, buffer_size, progress)

    with Executor(handler_factory, max_workers, name=name) as executor:
        with src_factory() as src:
            if progress:
                progress.size = src.size()
            if dirty:
                _copy_dirty(src, executor, progress)
            else:
                _copy_data(src, executor, zero, progress)


def _copy_data(src, executor, zero=True, progress=None):
    """
    Generic copy that can work with both new destination image or exiting
...
def _copy_dirty(src, executor, progress=None):
    """
    This for incremental backup using qcow2 format, and we assume a new empty
    image, so we never want to zero anything.
    """
...
class Executor(object):
    ...
    def __init__(self, handler_factory, max_workers, name="executor"):
        self._name = name
        self._workers = []
        self._queue = Queue(max_workers)

        log.debug("Starting executor %s", self._name)
        for i in range(max_workers):
            worker_name = "{}/{}".format(name, i)
            w = Worker(handler_factory, self._queue, name=worker_name)
