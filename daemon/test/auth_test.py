...
def test_cancel_ongoing_operations():
    ...
    # Few connetions are using this ticket. Each running an operation.
    ops = []
    for i in range(4):
        ctx = Context()
        op = Operation()
    ...
    # All ongoing opeations are cancled.
...
def test_canceled_fail_run_after():
    ...
    # If ticket was canceled with while ongoing operations are runing, ticket
    # run will fail removing the operations.
    with pytest.raises(errors.AuthorizationError):
        ticket.run(op)
...
def test_remove_contex_missing():
    ticket = Ticket(testutil.create_ticket(ops=["read"]))
...
def test_remove_contex_error():
    ...
    ticket = Ticket(testutil.create_ticket(ops=["read"]))
    ticket.add_context(1, FailingContxt())

    ticket.cancel(timeout=0)

    # If closing a context fails, keep it. The ticket cannot be reomoved until
    # this context is removed successfuly.
    ...
    # Calling again will close and remove the context sucessfuly.
    ticket.remove_context(1)
...
def test_authorizer_remove_async():
    ...
    # Ticket is cancled, but not removed.
    auth.remove(ticket.uuid)
    assert ticket.canceled
    assert ticket.info()["connections"] == 1
...
def test_authorizer_canceled():
    ...
    # Cancelling the tickt disables any operation.
    ticket.cancel()
