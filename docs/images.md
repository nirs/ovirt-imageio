...
oVirt take care of attaching a disk to the host serving the transfer URL
and installing a ticket allowing access the transfer URL.

When you connect to imageio server, the first thing you should do is
send an OPTIONS request to learn about the capabilities of the server.
Based on the server capabilities, you upload or download data in an
...
transfer image data using the unix socket for improved throughput and
lower CPU usage.

### max_readers

If the server supports multiple connections and the ticket is specifying
a backend supporting multiple connections (nbd, file) it will report the
...
support multiple connections and using multiple writers may fail.

### max_writers

If the server supports multiple connections and the ticket is specifying
a backend supporting multiple connections (nbd) it will report the
maximum number of connections in that can write a single image
concurrently.
...
When using multiple writers, each writer should modify distinct byte
range. If two writes modify the same byte range concurrently they will
...
Downloads bytes {start} to {end} from the image associated with
...
Downloading entire image is not efficient; the recommended way to is to
get the image extents and download only the needed extents.
...
The extents API return information about image content and allocation
or about changed blocks during incremental backup.
...
- ``context``: zero|dirty - Specify ``zero`` if you want to get zero
  ...
  are available only during incremental backup. If not specified,
  defaults to ``zero``.
...
Properties:
- ``start``: the offset bytes from the start of the image.
- ``length``: the length bytes.
- ``zero``: true if the extent read as zero; false if the extent is
  data.
- ``hole``: true if the extent is unallocated areas in qcow2 image

### Dirty extent

Describes the change status of an extent during incremental backup. If
the ``dirty`` flag is true, the extent was modified and should be
downloaded in this incremental backup. If the ``dirty`` flag is false,
the content did not change and this extents should not be downloading
during this backup.
...
- "404 Not Found": if dirty extents was requested when image transfer
  is not part of incremental backup.
...
The first extents was modified and should be downloaded in this
incremental backup. The second extents was not modified and should not
be downloaded in this incremental backup.
...
Upload entire image and flushing data to storage:
...
Upload last 1g extent starting 5g, and flushing to storage:
...
Note that image uses only the start byte from the content range. The
length of the upload is taken from the "Content-Length" header which is
required.
...
Zeroing ensures that the specified byte range will read as zero after
the operation. Depending on the ticket "sparse" option and the
underlying storage capabilities, the operation may deallocate space
(punch hole), or zero the range.
...
Zero 4096 at offset 1 GiB without flushing data to storage:
...
Zero entire 100 GiB disk and flush changes to storage:
    ...
    curl -k -X PATCH \
        --data-binary '{"op": "zero", "offset": 0, "size": 107374182400, "flush": true}' \
        https://server:54322/images/{ticket-id}
...
- "416 Range Not Satisfiable": Range or Content-Range exceeds the image
  size. The available content range is specified in the
  ``Content-Range`` header. You can retry the request with corrected
  range.

- "403 Forbidden": The ticket does not exist, expired, or canceled. You
  should not retry this request.
...
For internal server errors you need to check imageio server logs at
